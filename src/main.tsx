...
import { askTripAgent, createExpense, createGuideGroup, createGuideLink, createNote, createPhotoAlbum, deleteExpense, deleteGuideGroup, deleteNote, deletePhoto, deletePhotoAlbum, joinTrip, signInWithEmail, supabase, updateMemberName, uploadPhoto, upsertAccommodation, verifyEmailOtp, type Accommodation, type CloudNote, type CloudPhoto, type GuideGroup, type GuideLink, type PhotoAlbum } from './lib/backend'
...
function App() {
  ...
  const removeExpense = async (id:string|number) => {
    if (backend.session && backend.tripId) { await deleteExpense(String(id)); await backend.refresh() }
    else setExpenses(rows=>rows.filter(row=>row.id!==id))
  }
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {tab === 'map' && <MapView />}
      {tab === 'expense' && <ExpenseView expenses={expenses} onAdd={addExpense} onDelete={removeExpense} cloud={Boolean(backend.trip)} memberNames={memberNames.length ? memberNames : ['我']} />}
...
function ExpenseView({ expenses, onAdd, onDelete, cloud, memberNames }: {expenses:Expense[];onAdd:(expense:Omit<Expense,'id'>)=>Promise<void>;onDelete:(id:string|number)=>Promise<void>;cloud:boolean;memberNames:string[]}){
  const [deleting,setDeleting]=useState<string|number|null>(null);const [dayFilter,setDayFilter]=useState('all');const shownExpenses=dayFilter==='all'?expenses:expenses.filter(x=>x.date===dayFilter);const total=expenses.reduce((s,x)=>s+x.amount,0); const [showForm,setShowForm]=useState(false); const [showSettlement,setShowSettlement]=useState(false); const count=Math.max(memberNames.length,1); const target=total/count
  const remove=async(expense:Expense)=>{if(!confirm(`确定删除“${expense.title}”这笔账单吗？`))return;setDeleting(expense.id);try{await onDelete(expense.id)}finally{setDeleting(null)}}
...
function GalleryView({tripId,photos,albums,cloud,memberCount,refresh,openAuth}:{tripId:string|null;photos:CloudPhoto[];albums:PhotoAlbum[];cloud:boolean;memberCount:number;refresh:()=>Promise<void>;openAuth:()=>void}){
  const [selected,setSelected]=useState('all');const [preview,setPreview]=useState<CloudPhoto|null>(null);const [uploadOpen,setUploadOpen]=useState(false);const [albumOpen,setAlbumOpen]=useState(false);const [busy,setBusy]=useState(false);const [error,setError]=useState('')
  ...
  const removePhoto=async(photo:CloudPhoto)=>{if(!confirm('确定删除这张照片吗？云端原图也会被删除，无法恢复。'))return;setBusy(true);setError('');try{await deletePhoto(photo);setPreview(null);await refresh()}catch(err){setError(err instanceof Error?err.message:'删除照片失败')}finally{setBusy(false)}}
  return <div className="page"><PageHeading eyebrow={`${memberCount} 人共享空间 · ${cloud?'云端原图':'登录后使用'}`} title="旅途图册" note="按相册分组收纳照片，上传时可直接选择分组。" action={cloud?<button className="primary-button" onClick={()=>setUploadOpen(true)}><Plus size={17}/>上传照片</button>:<button className="primary-button" onClick={openAuth}><Users size={17}/>登录后共享</button>} />
    ...
    {shown.length===0?<section className="empty-gallery"><div className="camera-orbit"><Camera size={38}/></div><h2>{photos.length?'这个分组还没有照片':'故事还没开始'}</h2><p>{cloud?'新建相册后，上传时选择对应分组，后期查看会更清晰。':'登录并加入旅程后可以共享图册。'}</p>{cloud&&<button className="outline-button" onClick={()=>setUploadOpen(true)}><Camera size={17}/>上传照片</button>}</section>:<div className="photo-grid">{shown.map((p,i)=><figure key={p.id}><button className="photo-preview-button" onClick={()=>setPreview(p)}><img src={p.signedUrl} alt={`旅途照片 ${i+1}`}/></button><button className="photo-delete-button" disabled={busy} aria-label="删除这张照片" title="删除照片" onClick={()=>removePhoto(p)}><Trash2 size={16}/></button></figure>)}</div>}
    {preview&&<div className="photo-lightbox" onClick={()=>setPreview(null)}><button className="lightbox-close" aria-label="关闭预览" onClick={()=>setPreview(null)}><X/></button><img src={preview.signedUrl} alt="照片大图预览" onClick={e=>e.stopPropagation()}/><button className="lightbox-delete" disabled={busy} onClick={e=>{e.stopPropagation();removePhoto(preview)}}><Trash2 size={17}/>删除这张</button></div>}
